...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        print('Predicting {} steps ahead'.format(configs.pred_len))
        ...
        for patch_size in configs.patch_sizes:
            ...
            self.layers.append(Layer(input_dim=configs.d_model,
                                     num_nodes=configs.enc_in, cuts=cuts,
                                     cut_size=patch_size, factorized=self.factorized))
            self.skip_generators.append(WeightGenerator(in_dim=cuts * configs.d_model, out_dim=256, number_of_weights=1,
                                                        mem_dim=configs.mem_dim, num_nodes=configs.enc_in, factorized=False))
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        x_enc /= stdev
        x = self.start_fc(x_enc.unsqueeze(-1))
        batch_size = x.size(0)
        skip = 0
        ...
        dec_out = self.projections(x).transpose(2, 1)
...
class Layer(nn.Module):
    def __init__(self, input_dim, num_nodes, cuts, cut_size, factorized):
        ...
        self.temporal_embeddings = nn.Parameter(torch.rand(cuts, 1, 1, self.num_nodes, 5), requires_grad=True)
