...
class SeasonalNorm(nn.Module):
    def __init__(self, cycle_len, cycle_num, d_model):
...
class MacroviewNorm(nn.Module):
    def __init__(self, sample_len, d_model):
        ...
        self.mean_weight =  nn.Sequential(
            nn.Conv2d(in_channels = d_model, out_channels = d_model, kernel_size = (1, sample_len)),
            nn.Conv2d(in_channels = d_model, out_channels = d_model * sample_len, kernel_size = 1)
        )
        
    def forward(self, x):
        b, c, n, t = x.shape
        sample_x = x[..., -self.sample_len:]
        mean = self.mean_weight(sample_x).reshape(b, c, self.sample_len, n).permute(0, 1, 3, 2)
        mean = F.pad(mean, mode='constant', pad=(t - self.sample_len, 0))
        norm_x = x - mean
        return norm_x, mean, (norm_x[..., -self.sample_len:] ** 2).mean()
...
class EncoderLayer(nn.Module):
    def __init__(self, d_model, seq_len, pred_len, cycle_len, short_period_len, series_num, kernel_size, long_term=1, short_term=1, seasonal=1, spatial=1, dropout=0.1):
        ...
        if seasonal:
            self.cycle_norm = SeasonalNorm(cycle_len, seq_len // cycle_len, d_model)
        ...
        self.macro_norm = MacroviewNorm(short_period_len, d_model)
        self.macro_residual_ext = ResidualExtrapolate(d_model, short_period_len, pred_len)
        self.macro_ext = ResidualExtrapolate(d_model, short_period_len, pred_len)
        if short_term:
            ...
            self.short_period_residual_ext = ResidualExtrapolate(d_model, short_period_len, pred_len)
        ...
        if spatial:
            ...
            self.spatial_residual_ext = ResidualExtrapolate(d_model, short_period_len, pred_len)
        ...
        self.post_cycle_norm = SeasonalNorm(cycle_len, seq_len // cycle_len, d_model)
    ...
    def forward(self, x):
        ...
        #x, macro_mean, reg_loss = self.macro_norm(x)
        #macro_proj = self.macro_residual_ext(x)
        #macro_mean_proj = self.macro_ext(macro_mean)
        #xs.extend([macro_proj])
        ...
        s = self.skip_conv(s)
        #_, x_z, _ = self.post_cycle_norm(x_z)
        return x_z, s
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        input = torch.cat([(input > 0) * input, (input <= 0) * input], dim=1)
        in_len = input.size(3)
        ...
        for i in range(self.e_layers):
            residual = x
            x, s, reg_loss = self.enc_layers[i](x)
            x = x + residual
            out = s + out
        if self.ext:
