...
class DataEmbedding(nn.Module):
    ...
    def forward(self, x, lap_mx, tdh, dwh):
        ...
        x += self.spatial_embedding(lap_mx.cuda())
        x = self.dropout(x)
        return x
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        adj_mx = np.where(np.isnan(adj_mx), 1, np.where(adj_mx != 0, 1, 1)).astype('float32') 
        
        self.sh_mx = torch.FloatTensor(load_rel_2(adj_mx))
    ...
    def forward(self, input,  x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        for i, encoder_block in enumerate(self.encoder_blocks):
            enc = encoder_block(enc, TH, self.geo_mask.cuda(), self.sem_mask)
