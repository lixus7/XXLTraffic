...
class AGCRNCell(nn.Module):
    ...
    def forward(self, x, state, node_embeddings):
        ...
        # print(f"After layer4: {hc.mean().item()}, {hc.std().item()}")
        h = r*state + (1-r)*hc
        return h
...
class Model(nn.Module):
    def __init__(self, configs, input_dim=1, output_dim=1, rnn_units=32, num_layers=2, default_graph=True, embed_dim=8, cheb_k=2):
        ...
        self.encoder = AVWDCRNN(self.num_node, input_dim, rnn_units, cheb_k, embed_dim, num_layers)

        #predictor
        self.end_conv = nn.Conv2d(1, self.horizon * self.output_dim, kernel_size=(1, self.hidden_dim), bias=True)

    # def forward(self, source, targets, teacher_forcing_ratio=0.5):
    def forward(self, source, x_mark_enc, x_dec, x_mark_dec, mask=None):
...
#         output = output.permute(0, 1, 3, 2).squeeze(-1)                             #B, T, N, C
#         # output shape
